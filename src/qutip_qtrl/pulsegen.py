...
class PulseGenGrafs(PulseGen):
    ...
    def set_optim_var_vals(self, param_vals):
        """
        Get the parameter values to be optimized
        Returns
        -------
        list (or 1d array) of floats
        """
    ...
    def set_coeffs(self, param_vals):
        """
        Set the values of the 1-d (length num_basis_funcs) array of coefficients
        """
        self.coeffs = param_vals
    
    def get_pulse(self):
        """
        Get pulse generated by specified basis function
        Returns
        -------
        ndarray : matrix of basis functions (num_tslots x num_basis_funcs)
        """
        return self.gen_pulse()

class PulseGenGrafsSlepian(PulseGenGrafs):
    """
    Generates a pulse using the Slepian basis functions
    Attributes
    ----------
    freqs : float array[num_coeffs]
        Frequencies for the basis functions
